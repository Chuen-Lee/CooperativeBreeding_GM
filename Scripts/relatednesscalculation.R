...
genotypedata %>% group_by(Method) %>% distinct(Locus,.keep_all = T) %>% summarise(n=n())
...
#write_delim(gtd2,"~/Documents/PhD/R_analysis/CooperativeBreeding_CL/gtd.txt",delim=" ", col_names = F) # remove comment to write

swgtd <- readgenotypedata("~/Documents/PhD/R_analysis/CooperativeBreeding_CL/gtd.txt")

relatednessperbird <- coancestry(swgtd$gdata, quellergt=2)

relatednessperbirdfreqs <- relatednessperbird$freqs

relatednessbi <- relatednessperbird$relatedness %>% dplyr::select(ind1.id,ind2.id,quellergt)
#write_csv(relatednessbi,"InputTables/microstatrelatedness.csv") # remove comment to write
